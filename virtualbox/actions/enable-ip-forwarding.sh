...
\nset -x
\n/sbin/iptables -L -n -t nat | grep $host_nat_network
\nif [ \$? -eq 1 ]; then
\n/sbin/iptables -t nat -A POSTROUTING -o $(ip r | grep default | cut -f5 -d ' ') -s $host_nat_network/24  ! -d $host_nat_network/24 -j MASQUERADE
\nfi
EOF
)
