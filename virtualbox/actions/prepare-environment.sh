...
type ssh >/dev/null 2>&1 || { echo >&2 "SSH client is not installed. Please install \"openssh\" package if you run this script under Cygwin. Aborting"; exit 1; }
echo "OK"
...
case "$(uname)" in
  ...
  CYGWIN*)
    ...
    type ipconfig >/dev/null 2>&1 || { echo >&2 "No ipconfig available in Cygwin environment. Please check you can run ipconfig from Cygwin command prompt."; exit 1; }
    echo "OK"
  ;;
  *)
