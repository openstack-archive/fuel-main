...
create_hostonly_interface() {
  ...
  # Exit if the interface already exists (deleting it here is not safe, as 
  # VirtualBox creates hostonly adapters sequentially)
...
delete_all_hostonly_interfaces() {
  OIFS=$IFS;IFS=",";list=(`VBoxManage list hostonlyifs | grep '^Name' | \
    sed 's/^Name\:[ \t]*//' | uniq | tr "\\n" ","`);IFS=$OIFS
