...
wait_for_product_vm_to_download() {
    ...
    maxdelay=3000
    while ! wait_for_line_in_puppet_bootstrap $ip $username $password "$prompt" "build docker containers finished.|Deployment task has succeeded: cobbler|^Fuel.*complete" "^Fuel.*FAILED"; do
        sleep 5
        ((waited += 5))
...
enable_outbound_network_for_product_vm() {
    ...
    result=$(
        ...
        expect "$prompt"
        # eth2 should get ip addrees via dhcp and used default route
        ...
        expect "$prompt"
        # restart cobbler container in the docker mode, or dnsmasq serviece in the dockerless mode
        ...
        expect "$prompt"
        # restart again with new newtwork settings
        send "dockerctl restart cobbler &>/dev/null ; service dnsmasq restart &>/dev/null\r"
        set timeout 300
        expect "$prompt"
        send "logout\r"
