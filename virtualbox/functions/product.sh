...
enable_outbound_network_for_product_vm() {
    ...
    result=$(
        ...
        expect "$prompt"
        # remove old settings from the resolv.conf if exists
        send "sed -i.orig '/^nameserver/d' /etc/resolv.conf &>/dev/null\r"
        expect "$prompt"
        # update the resolv.conf with the new settings
        send "echo -e '$nameserver' >>/etc/resolv.conf\r"
        expect "$prompt"
