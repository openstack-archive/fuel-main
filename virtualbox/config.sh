...
vm_slave_memory_mb[1]=1024  # for controller node at least 1Gb is required if you also run Ceph on it
vm_slave_memory_mb[2]=1024  # for compute node 1GB is recommended, otherwise VM instances in OpenStack may not boot
vm_slave_memory_mb[3]=1024  # for a dedicated Cinder node 768 MB should be sufficient, but 1Gb is
                            #   required if you also run Ceph on it
