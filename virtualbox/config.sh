...
for ip in 10.20.0.1 172.16.1.1 172.16.0.1 192.168.0.1 192.168.1.1; do
# VirtualBox for Windows has different virtual NICs naming and indexing
  if [ $idx -eq 0 ]; then
    case "$(uname)" in
      ...
      ;;
      *)
        printf '%s\n' "$(uname) is not supported operating system."
        exit 1
      ;;
    esac 
  else
...
# Fuel 3.2 nodes require 1.5GiB of RAM as absolute minimum!
vm_slave_memory_default=1536
vm_slave_memory_mb[1]=1536   # for controller node 768 MB should be sufficient
vm_slave_memory_mb[2]=1536  # for compute node 1GB is recommended, otherwise VM instances in OpenStack may not boot
vm_slave_memory_mb[3]=1536   # for a dedicated Cinder node 768 MB should be sufficient
