...
DIFF_CENTOS_DEP_FILE:=$$(call find-files,$(DEPS_DIR)/$1/$$(DIFF_CENTOS_REPO_ART_NAME))
ifneq ($$(DIFF_CENTOS_DEP_FILE),)
$(ARTS_DIR)/$$(DIFF_CENTOS_REPO_ART_NAME): $$(DIFF_CENTOS_DEP_FILE)
	@mkdir -p $$(@D)
	cp $$< $$@
else
...
CENTOS_DEP_FILE:=$$(call find-files,$(DEPS_DIR)/$1/$(CENTOS_REPO_ART_NAME))
ifneq ($$(CENTOS_DEP_FILE),)
$(BUILD_DIR)/mirror/$$(DIFF_CENTOS_REPO_ART_NAME): NEWDIR=$(BUILD_DIR)/mirror/$1/centos-repo/Packages
$(BUILD_DIR)/mirror/$$(DIFF_CENTOS_REPO_ART_NAME): $(BUILD_DIR)/mirror/centos_repo_$1.done
$(BUILD_DIR)/mirror/centos_repo_$1.done: $$(CENTOS_DEP_FILE)
	mkdir -p $(BUILD_DIR)/mirror/$1
	tar xf $$(CENTOS_DEP_FILE) -C $(BUILD_DIR)/mirror/$1
	@mkdir -p $$(@D)
	touch $$@
else
$(BUILD_DIR)/mirror/$$(DIFF_CENTOS_REPO_ART_NAME): NEWDIR=$(LOCAL_MIRROR_CENTOS_OS_BASEURL)/Packages
$(BUILD_DIR)/mirror/$$(DIFF_CENTOS_REPO_ART_NAME): $(BUILD_DIR)/mirror/centos_repo_$1.done
$(BUILD_DIR)/mirror/centos_repo_$1.done: \
...
DIFF_UBUNTU_DEP_FILE:=$$(call find-files,$(DEPS_DIR)/$1/$$(DIFF_UBUNTU_REPO_ART_NAME))
ifneq ($$(DIFF_UBUNTU_DEP_FILE),)
$(ARTS_DIR)/$$(DIFF_UBUNTU_REPO_ART_NAME): $$(DIFF_UBUNTU_DEP_FILE)
	@mkdir -p $$(@D)
	cp $$< $$@
else
...
UBUNTU_DEP_FILE:=$$(call find-files,$(DEPS_DIR)/$1/$(UBUNTU_REPO_ART_NAME))
ifneq ($$(UBUNTU_DEP_FILE),)
$(BUILD_DIR)/mirror/$$(DIFF_UBUNTU_REPO_ART_NAME): NEWDIR=$(BUILD_DIR)/mirror/$1/ubuntu-repo/Packages
$(BUILD_DIR)/mirror/$$(DIFF_UBUNTU_REPO_ART_NAME): $(BUILD_DIR)/mirror/ubuntu_repo_$1.done
$(BUILD_DIR)/mirror/ubuntu_repo_$1.done: $$(UBUNTU_DEP_FILE)
	mkdir -p $(BUILD_DIR)/mirror/$1
	tar xf $$(UBUNTU_DEP_FILE) -C $(BUILD_DIR)/mirror/$1
	@mkdir -p $$(@D)
	touch $$@
else
$(BUILD_DIR)/mirror/$$(DIFF_UBUNTU_REPO_ART_NAME): NEWDIR=$(LOCAL_MIRROR_UBUNTU_OS_BASEURL)/Packages
$(BUILD_DIR)/mirror/$$(DIFF_UBUNTU_REPO_ART_NAME): $(BUILD_DIR)/mirror/ubuntu_repo_$1.done
$(BUILD_DIR)/mirror/ubuntu_repo_$1.done: \
		$(BUILD_DIR)/mirror/build.done \
		$(BUILD_DIR)/packages/build.done \
		$(BUILD_DIR)/openstack/build.done
	@mkdir -p $$(@D)
