...
# Grab every udeb
for udeb in $(wget -qO - http://mirrors.msk.mirantis.net/ubuntu/dists/precise/main/debian-installer/binary-amd64/Packages.bz2 | bzip2 -cd | sed -ne 's/^Filename: //p'); do
        wget -N http://mirrors.msk.mirantis.net/ubuntu/$udeb
done
...
for idx in override.precise.main override.precise.extra.main override.precise.main.debian-installer ; do
	wget -N http://mirrors.msk.mirantis.net/ubuntu/indices/$idx
done
cd /repo
