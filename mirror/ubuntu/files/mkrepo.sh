...
rm -f ${wrkdir}/UPackages.tmp ${wrkdir}/override.precise.main ${wrkdir}/override.precise.extra.main ${wrkdir}/override.precise.main.debian-installer
touch ${wrkdir}/UPackages.tmp ${wrkdir}/override.precise.main ${wrkdir}/override.precise.extra.main ${wrkdir}/override.precise.main.debian-installer
...
for list in /etc/apt/sources.list /etc/apt/sources.list.d/*.list; do
  for repo in `cat $list| grep -v backports | sed 's| \+|\||g' | grep "^deb|"`; do
     ...
     for repo in $repos; do
       ...
       # Getting indices
       wget -N ${repourl}/indices/override.${repodist}.${repo} >> ${wrkdir}/override.precise.main
       wget -N ${repourl}/indices/override.${repodist}.extra.${repo} ${wrkdir}/override.precise.extra.main
       wget -N ${repourl}/indices/override.${repodist}.${repo}.debian-installer ${wrkdir}/override.precise.main.debian-installer
     done
  done
done
...
for i in $gvpackages; do
  packages="$packages `cat ${wrkdir}/UPackages.tmp | egrep "generic|virtual" | egrep $i | sort -rV | head -1`"
done
...
for idx in override.precise.main override.precise.extra.main override.precise.main.debian-installer ; do
  cat ${wrkdir}/$idx | sort -u > ${wrkdir}/repo/indices/$idx
done
