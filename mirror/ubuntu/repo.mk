...
requirements_deb:=$(shell cat $(SOURCE_DIR)/requirements-deb.txt)
...
$(per_pkg_downloads_urls): $(workdir)/urllists/%.list: $(apt_setup_done)
	...
	if ! apt-get -c "$(apt_alt_conf)" --print-uris --yes -qq install $* >$@.tmp 2>"$${apt_log}"; then \
		echo "package $* can not be installed" >> "$${apt_log}"; \
		apt-get -c "$(apt_alt_conf)" -o Debug::pkgProblemResolver=true  --print-uris --yes install $* >>"$${apt_log}" 2>&1; \
		exit 1; \
	fi; \
	mv $@.tmp $@
