...
$(BUILD_DIR)/mirrors/centos/src_urls.list: $(BUILD_DIR)/mirror/centos/urls.list
	log="$(BUILD_DIR)/mirror/centos/yumdownloader_src.log"; \
	yumdownloader --urls --archlist=src --source \
		-c $(BUILD_DIR)/mirror/centos/etc/yum.conf \
		$(REQUIRED_RPMS) > $@.tmp 2>$$log && \
	mv $@.tmp $@
