...
def is_expired(token):
    return time.gmtime() > time.strptime(token.expires, TIME_FORMAT)
...
class NailgunClient(object):
    def __init__(self, admin_node_ip, **kwargs):
        ...
        self.keystone = self.authenticate(keystone_url, self.creds)
        self.refresh_token()
    ...
    def authenticate(self, url):
        try:
            keystone = keystoneclient(auth_url=url, **self.creds)
    ...
    def refresh_token(self):
        self.token = self.keystone.tokens.authenticate(**self.creds)
        self.client.reset_token(self.token.id)
