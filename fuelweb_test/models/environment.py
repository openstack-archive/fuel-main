...
class EnvironmentModel(object):
    ...
    def get_admin_remote(self, login=settings.SSH_CREDENTIALS['login'],
        ...
        :rtype : SSHClient
        """
        return self.get_virtual_environment().nodes_container().admin.remote(
    ...
    def get_admin_node_ip(self):
        return str(
            self.get_virtual_environment().nodes_container(
    ...
    def make_snapshot(self, snapshot_name, description="", is_make=False):
        if settings.MAKE_SNAPSHOT or is_make:
            ...
            try:
                self.get_virtual_environment().nodes_container().admin.await(
    ...
    def revert_snapshot(self, name):
        if self.get_virtual_environment().has_snapshot(name):
            ...
            admin = self.get_virtual_environment().nodes_container().admin
            ...
            try:
                ...
                self.get_virtual_environment().start(
                    self.get_virtual_environment().nodes_container().admins)
            ...
            for node in self.get_virtual_environment().nodes_container(
            ).slaves:
    ...
    def describe_second_admin_interface(self):
        remote = self.get_admin_remote()
        second_admin_network = self.get_network(self.get_virtual_environment(
        ).admin_net2).split('/')[0]
        ...
        second_admin_ip = str(self.get_virtual_environment().nodes_container(
