...
class EnvironmentModel(object):
    ...
    def describe_environment(self):
        ...
        for name in interfaces:
            networks.append(
                self.get_virtual_environment().create_networks(name))
        for name in self.get_virtual_environment().node_roles.admin_names:
            self.get_virtual_environment().describe_admin_node(name, networks)
        for name in self.get_virtual_environment().node_roles.other_names:
            if settings.MULTIPLE_NETWORKS:
                ...
                if int(name[-2:]) % 2 == 1:
                    self.get_virtual_environment().describe_empty_node(
                        name, networks1)
                elif int(name[-2:]) % 2 == 0:
                    self.get_virtual_environment().describe_empty_node(
                        name, networks2)
    ...
    def nodes(self):
        return Nodes(self.get_virtual_environment(),
                     self.get_virtual_environment().node_roles)
