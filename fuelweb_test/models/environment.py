...
class EnvironmentModel(object):
    ...
    def configure_second_admin_firewall(self, network, netmask):
        ...
        cmd = ('iptables -t nat -I POSTROUTING -s {0}/{1} -p udp -m udp --dpo'
               'rt 514 -m comment --comment "rsyslog-udp-514-unmasquerade" -j'
               ' ACCEPT;iptables -t nat -I POSTROUTING -s {0}/{1} -p tcp -m '
               'tcp --dport 514 -m comment --comment "rsyslog-tcp-514-unmasqu'
               'erade" -j ACCEPT; iptables -A POSTROUTING -s {0}/{1} -o eth+ '
               '-m comment --comment "004 forward_admin_net2" -j MASQUERADE;'
               'iptables I FORWARD -i eth5 -o docker0 -p tcp -m state --state'
               'NEW -m tcp --dport 514 -m comment --comment "rsyslog-'
               'tcp-514-accept" -j ACCEPT ; iptables -I FORWARD -i eth5 -o do'
               'cker0 -p udp -m state --state NEW -m udp --dport 514 -m comme'
               'nt --comment "rsyslog-udp-514-accept" -j ACCEPT;  service ipta'
               'bles save;').format(network, netmask)
        result = remote.execute(cmd)
        assert_equal(result['exit_code'], 0, ('Failed to add firewall rules f'
                                              'or second admin network on mas'
                                              'ter node: {0}').format(result))
