...
class EnvironmentModel(object):
    ...
    def sync_node_time(self, remote):
        ...
        ntpd_exit = remote.execute("ntpdate -d $(awk '/^server/{print $2}'"
                                   " /etc/ntp.conf)")
        assert_equal(0, ntpd_exit['exit_code'], 'Failed to sync time: {0}'.
                     format(ntpd_exit['stderr']))
    ...
    def sync_time_admin_node(self):
        ...
        try:
            ...
            self.execute_remote_cmd(remote, 'ntpd -qg')
            self.execute_remote_cmd(remote, 'service ntpd start')

        # Wait up to 4 minutes till NTPD synced time and ready for connections
        cmd = "ntpdate -d 127.0.0.1"
        try:
            wait(lambda: remote.execute(cmd)['exit_code'] == 0, 30, 240)
            logger.debug('Synced time from localhost, NTPD is ready')
        except Exception as e:
            logger.error('Failed to sync time from localhost using'
                         'ntpdate: {0}'.format(e))
            raise
