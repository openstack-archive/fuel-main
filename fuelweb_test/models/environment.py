...
class EnvironmentModel(object):
    ...
    def revert_snapshot(self, name):
        if self.get_virtual_environment().has_snapshot(name):
            ...
            time.sleep(10)
            try:
                self.sync_node_time(self.get_ssh_to_remote_by_name(name))
                logging.info('Syncing time for node %s after reverting to snapshot' % name)
            except:
                logging.warn('Something goes wrong on time sync for node %s' % name)
            return True
        return False
