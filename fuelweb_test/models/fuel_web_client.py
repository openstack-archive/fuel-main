...
class FuelWebClient(object):
    ...
    @logwrap
    def verify_network(self, cluster_id, timeout=60 * 5):
    ...
    def backup_master(self, remote):
        ...
        try:
            remote.execute("echo CALC_MY_MD5SUM > /root/data")
            remote.execute("iptables-save > /root/iptables-backup")
            remote.execute("md5sum /root/data | sed -n 1p "
                           "| awk '{print $1}'>/root/sum")
