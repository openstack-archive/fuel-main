...
class CustomRepo(object):
    ...
    def prepare_repository(self):
        ...
        if not self.custom_pkgs_mirror:
            return
        logger.info("Custom mirror with new packages: {}"
        ...
        if settings.OPENSTACK_RELEASE_UBUNTU in settings.OPENSTACK_RELEASE:
            ...
            master_tools = ['http://osci-obs.vm.mirantis.net:82/'
                            'centos-fuel-master/centos/x86_64/'
                            'dpkg-1.15.5.6-6.el6.x86_64.rpm',
                            'http://osci-obs.vm.mirantis.net:82/'
                            'centos-fuel-master/centos/noarch/'
                            'dpkg-devel-1.15.5.6-6.el6.noarch.rpm']
    ...
    def modify_resolv_conf(self, nameservers=[]):
        ...
        logger.debug('echo "{0}" > /etc/resolv.conf'
                     .format(resolv_new))
        echo_result = remote.execute('echo "{0}" > /etc/resolv.conf'
                                     ''.format(resolv_new))
    ...
    def install_tools(self, master_tools=[]):
        logger.info("Installing necessary tools for {}"
    ...
    def get_pkgs_list_centos(self):
        ...
        if not lists_location:
            logger.error('CentOS mirror error: Could not parse {}'
                         .format(url))
            raise
        ...
        if '.xml.gz' in lists_location:
            try:
                ...
                logger.error('CentOS mirror error: Could not decompress {}'
                             .format(url))
                raise
    ...
    def download_pkgs(self, pkgs_local_path):
        ...
        for pkg in self.pkgs_list:
            ...
            wget_cmd = "wget --directory-prefix {0} {1}/{2}"\
        ...
        for pkg in self.pkgs_list:
            ...
            if result['exit_code'] == 0:
                ...
                if result['exit_code'] == 1:
                    ...
                    logger.error('Error updating {}'.format(yaml_versions))
                    raise
