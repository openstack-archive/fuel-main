...
function node_apt_repo {
  node=$1
  repourl=$2
  ssh $node "echo 'deb $repourl' >> /etc/apt/sources.list"
}
...
if [[ $repotype == "yum" ]]; then
  updatecommand="yum clean expire-cache; yum update -y $packages -x ruby*;"
else
  updatecommand="apt-get update;apt-get update -y $packages"
fi
