...
repobasepath="/var/www/nailgun/2014.2-6.1"
tempdownloaddir=$(mktemp -d /tmp/repo.XXXX)
LOGFILE=/var/log/repomirror.log
packages=""

#FIXME(mattymo): add to puppet packages
yum install -y yum-utils dpkg dpkg-devel createrepo
...
function node_apt_repo {
  node=$1
  repourl=$2
  echo deb $repourl / | ssh $node "cat >> /etc/apt/sources.list"
}

function node_yum_repo {
  node=$1
  repourl=$2
  ssh $node bash -c "yum-config-manager --add-repo $repourl" > /dev/null
}
...
function get_node_list {
  while read nodeid; do
    echo "node-$nodeid"
  done < <(fuel nodes --env $1 | tail -n +3 | cut -d' ' -f1)
}
...
if [ -z "$newreponame" ]; then
  newreponame="fuel-6.1-custom"
fi
...
if [[ $repotype == "yum" ]]; then
  updatecommand="yum clean expire-cache;yum install $packages -y;yum update -y $packages -x ruby*;"
else
  updatecommand="apt-get update;apt-get install -y $packages"
fi
