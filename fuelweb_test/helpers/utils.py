...
@logwrap
def check_if_service_restarted_ubuntu(node_ssh, services_list=None, skip=0):
    if services_list:
        cmd = ("tail -n +{1} {0}| grep -E '/sbin/(re)?start' |"
               "awk -F' ' '{{print $11}}'"
               .format('/var/log/puppet.log', skip))
...
@logwrap
def check_if_service_restarted_centos(node_ssh, services_list=None, skip=0):
    if services_list:
        cmd = ("tail -n +{1} {0} | grep -E '/sbin/service openstack-%s "
               "(re)?start' | awk -F' ' '{{print $11}}'"
               .format('/var/log/puppet.log', skip))
        for service in services_list:
            res = node_ssh.execute(cmd % service)['stdout']
            logger.debug('Next services were restarted {0}'.format(res))
            asserts.assert_true(len(res) > 0,
