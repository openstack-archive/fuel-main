...
def restore_check_sum(remote):
    ...
    md5sum_backup = remote.execute("cat /root/sum")
    md5sum_restore = remote.execute("md5sum /root/data | sed -n 1p "
                                    " | awk '{print $1}'")
...
def iptables_check(remote):
    ...
    aaa = remote.execute("iptables -L")["stdout"]
    logger.info('!!!!iptables!!!! is: {}'.format(aaa))

    remote.execute("iptables-save > /root/iptables-restore")
    #logger.info('firewall_save is: {}'.format(save))
    #iptables_backup = remote.execute("sed -e '/^:/d; /^#/d' "
    #                                 " /root/iptables-backup")["stdout"]
    #iptables_restore = remote.execute("sed -e '/^:/d; /^#/d' "
    #                                  " /root/iptables-restore")["stdout"]
    #logger.info('iptables_restore is: {}'.format(iptables_restore))

    assert_equal(iptables_backup, iptables_restore,
                 "list of chains are not equal")
