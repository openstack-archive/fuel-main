...
def check_upgraded_containers(remote, version_from, version_to):
    ...
    for i in range(1, len(containers)):
        assert_true(version_from not in containers[i],
                    'There are {0} containers'.format(version_from))
    for i in range(1, len(containers)):
        assert_true(version_to in containers[i],
                    'There are no {0} containers'.format(version_to))
        assert_true('Up' in containers[i],
                    'Container {0} is down'.format(containers[i]))
...
def check_containers_work(remote):
    ...
    assert_true('Exited' in nginx,
                'container status is not Exited, it is {0}'.format(nginx))
    remote.execute('"dockerctl start nginx"')
    nginx = remote.execute('docker ps -a | grep nginx')['stdout']
    logger.debug('nginx status after start is {0}'.format(nginx))
    assert_true('Up' in nginx,
                'container status is not Up, it is {0}'.format(nginx))
...
def run_script(node_ssh, script_path, script_name):
    ...
    assert_equal(chan.recv_exit_status(), 0,
                 'Upgrade script fails with next message {0}'.format(chan.recv_exit_status()))
