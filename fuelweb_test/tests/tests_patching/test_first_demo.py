...
class TestPatching(TestBasic):
    ...
    def first_patching_demo(self):
        ...
        tests_parent_dir = ('{0}/patching_tests/bug{1}/tests'
                      .format(os.environ.get("WORKSPACE", "./"),bug_id))
        ...
        self.fuel_web.warm_start_nodes([self.env.nodes().slaves[0]])
        self.fuel_web.warm_start_nodes(self.env.nodes().slaves[1:3])

        # Manually start cinder-volume on slaves (LP#1421595)
        for node in self.env.nodes().slaves[1:3]:
            ...
            self.fuel_web.wait_cinder_is_up([node.name])
        ...
        for filename in os.listdir(tests_parent_dir):
            result=admin.execute("cd {0}; chmod +x {1} && {2}".
                                 format(tests_dir_on_master,
                                        filename, filename))
            assert_equal(0, result['exit_code'], "Command failed with error: {0}".
                     format(result))
