...
class CephRadosGW(base_test_case.TestBasic):
    ...
    def ceph_rados_gw(self):
        ...
        proboscis.asserts.assert_true(radosgw_started(), 'radosgw daemon started')
...
@proboscis.test(groups=["thread_1", "ceph"])
class VmBackedWithCephMigrationBasic(base_test_case.TestBasic):
    ...
    def migrate_vm_backed_with_ceph(self):
        ...
        srv = os.create_server(scenario='./fuelweb_test/helpers/'
        ...
        if srv.status != "ERROR":
            ...
            if md5after not in md5before:
                logger.info("Md5 checksums don`t match.")
                logger.info(
...
@proboscis.test(groups=["thread_1", "ceph"])
class VmBackedWithCephMigrationVolume(base_test_case.TestBasic):
    ...
    def migrate_vm_backed_with_ceph(self):
        ...
        Scenario:
            ...
            4. Deploy the cluster
            5. Check ceph status
            8. Create a new VM, assign floating ip
            9. Create a new volume, attach it to VM
            10. Create filesystem and mount volume.
            8. Migrate VM
            9. Check cluster and server state after migration
            10. Terminate VM

        Snapshot vm_backed_with_ceph_live_migration_with_volume
        ...
        if srv.status != "ERROR":
            ...
            vol = os.attach_volume(vol, srv)

            time.sleep(100)
            logger.info("Create filesystem and mount volume")
            os.execute_through_host(
                self.env.get_ssh_to_remote_by_name('slave-01'),
                floating_ip.ip, '. /home/mount_volume.sh', creds)

            os.execute_through_host(
                self.env.get_ssh_to_remote_by_name('slave-01'),
                floating_ip.ip, 'touch /mnt/file-on-volume', creds)
            ...
            if "file-on-volume" not in os.execute_through_host(
                    self.env.get_ssh_to_remote_by_name('slave-01'),
                    floating_ip.ip, "ls /mnt", creds):
                logger.info("File is abscent in /mnt")
                raise Exception()
        ...
        self.env.make_snapshot("vm_backed_with_ceph_live_migration_with_volume")
