...
class VmBackedWithCephMigrationBasic(TestBasic):
    ...
    def migrate_vm_backed_with_ceph(self):
        ...
        srv_remote_node = self.fuel_web.get_ssh_to_remote_by_instance(os, srv)
        srv_instance_ip = self.fuel_web.get_instance_ip(srv)
        srv_instance_mac = self.fuel_web.get_instance_mac(os, srv)
        srv_node_dhcp_ip = self.fuel_web.get_node_dhcp_ip(os, srv)
        ...
        wait(lambda: tcp_ping(floating_ip.ip, 22), timeout=1000)
        ...
        wait(lambda: tcp_ping(floating_ip.ip, 22), timeout=100)
        ...
        # Checking if dhcp lease for instance still remains
        # on previous compute node. Related Bug: #1391010
