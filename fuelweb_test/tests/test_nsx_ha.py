...
class NSXTestsInHAMode(TestBasic):
    ...
    @test(depends_on=[SetupEnvironment.prepare_slaves_5],
          groups=["nsx_ha_stt_stop_deployment"])
    @log_snapshot_on_error
    def nsx_ha_stt_stop_deployment(self):
            ...
            Snapshot nsx_HA_stt_stop_deployment
            ...
            self.fuel_web.run_ostf(cluster_id=cluster_id,
                                   test_sets=['ha', 'smoke', 'sanity'])
            self.env.make_snapshot("nsx_ha_stt_stop_deployment")
    ...
    @test(depends_on=[SetupEnvironment.prepare_slaves_7],
          groups=["nsx_ha_stt_add_node"])
    @log_snapshot_on_error
    def nsx_ha_stt_add_node(self):
        ...
        self.fuel_web.run_ostf(cluster_id=cluster_id,
                               test_sets=['ha', 'smoke', 'sanity'])
        self.env.make_snapshot("nsx_ha_stt_add_node")
