    ...
    def vcenter_multiple_cluster(self):
        ...
        for hypervisor in hypervisors:
            assert_true(os.get_hypervisor_vms_count(hypervisor) != 0, 
                        "No active VMs on " + os.get_hypervisor_hostanme(hypervisor))
            logger.info(str(os.get_hypervisor_vms_count(hypervisor)) + 
                        " active VMs  on Hypervisor " + 
        ...
        res = os.execute_through_host(self.env.get_ssh_to_remote_by_name("slave-01"), 
                                      ips_for_check[0], "ping -q -c3 " + ips_for_check[1] + " | grep received | grep -v '0% packet loss'")
