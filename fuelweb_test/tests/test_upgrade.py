...
from fuelweb_test.tests.test_simple import SimpleCinder
from fuelweb_test.tests.test_neutron import NeutronGreHa
from fuelweb_test.tests.test_neutron import NeutronGre
from fuelweb_test.tests.base_test_case import TestBasic
from fuelweb_test import logger
from fuelweb_test.settings import TARBALL_LOCAL_PATH
from fuelweb_test.settings import TARBALL_NAME


@test(groups=["upgrade"])
class UpgradeFuelMaster(TestBasic):
    ...
    def upgrade_simple_env(self):
        ...
        checkers.upload_tarball(self.env.get_admin_remote(), '/'.join([TARBALL_LOCAL_PATH, TARBALL_NAME]), '/var')
        checkers.check_tarball_exists(self.env.get_admin_remote(), TARBALL_NAME, '/var')
        checkers.untar(self.env.get_admin_remote(), TARBALL_NAME, '/var')
        checkers.run_script(self.env.get_admin_remote(), '/var', 'upgrade.sh')
        checkers.wait_upgrade_is_done(self.env.get_admin_remote(), 1500)
        checkers.check_upgraded_containers(self.env.get_admin_remote(), '5.0', '5.1')
    ...
    @test(depends_on=[NeutronGreHa.deploy_neutron_gre_ha],
          groups=["upgrade_ha"])
    @log_snapshot_on_error
...
class RollbackFuelMaster(TestBasic):
    @test(depends_on=[SimpleCinder.deploy_simple_cinder],
    ...
    def rollback_simple_env(self):
        ...
        checkers.check_upgraded_containers(self.env.get_admin_remote(), '5.1', '5.0')
        self.fuel_web.run_ostf(cluster_id=cluster_id)

        self.env.make_snapshot("rollback_manual")

    @test(depends_on=[NeutronGre.deploy_neutron_gre],
    ...
    def rollback_automatically_simple_env(self):
        ...
        self.fuel_web.modify_python_file(self.env.get_admin_remote(),
                                         "69i \ \ \ \ \ \ \ \ raise errors.ExecutedErrorNonZeroExitCode('{0}')"
                                         .format('Some bad error'),
                                         '/var/upgrade/site-packages/fuel_upgrade/engines/docker_engine.py')
        checkers.run_with_rollback(self.env.get_admin_remote(), '/var', 'upgrade.sh')
