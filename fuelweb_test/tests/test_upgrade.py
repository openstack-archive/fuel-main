...
class UpgradeFuelMaster(base_test_data.TestBasic):
    ...
    def upgrade_simple_env(self):
        ...
        if not self.env.get_virtual_environment().has_snapshot('deploy_simple_cinder'):
            raise SkipTest()
    ...
    def upgrade_ha_env(self):
        ...
        """
        if not self.env.get_virtual_environment().has_snapshot('deploy_neutron_gre_ha'):
            raise SkipTest()
...
class RollbackFuelMaster(base_test_data.TestBasic):
    ...
    def rollback_automatically_simple_env(self):
        ...
        """
        if not self.env.get_virtual_environment().has_snapshot('deploy_neutron_gre'):
            raise SkipTest()
