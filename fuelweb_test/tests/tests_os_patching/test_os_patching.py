...
class TestPatch(TestBasic):
    ...
    @test
    #@test(depends_on=[test_simple.SimpleFlat.deploy_simple_flat], groups=["deploy_simple_flat_and_patch"])
    ...
    def deploy_and_patch(self):
        ...
        """
        logger.info("What we have in this function {0}".format(dir(self)))
        ...
        # Get cluster nodes
        nailgun_nodes = [self.fuel_web.get_nailgun_node_by_devops_node(node)
        ...
        checkers.wait_update_is_done(node_ssh=node_ssh,
        ...
        self.env.make_snapshot(self.snapshot, is_make=True)

@factory
def generate_user_tests():
    snap = os.environ.get('SNAPSHOT').split(",")
