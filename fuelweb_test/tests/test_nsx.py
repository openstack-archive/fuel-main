...
class NSX_Acceptance_Simple(TestBasic):
    ...
    @test(depends_on=[SetupEnvironment.prepare_slaves_3],
          groups=["nsx_simple_stt_normal"])
    @log_snapshot_on_error
    def nsx_simple_stt_normal(self):
        ...
        Snapshot nsx_simple_sst_normal
        ...
        self.env.make_snapshot("nsx_simple_sst_normal")

    @test(depends_on=[SetupEnvironment.prepare_slaves_5],
          groups=["nsx_simple_sst_add_node"])
    @log_snapshot_on_error
    def nsx_simple_sst_add_node(self):
        ...
        Snapshot nsx_simple_sst_add_node
        ...
        self.fuel_web.security.verify_firewall(cluster_id)
        self.fuel_web.run_ostf(cluster_id=cluster_id)
        self.env.make_snapshot("nsx_simple_sst_add_node")

    @test(groups=["nsx"])
    @test(depends_on=[SetupEnvironment.prepare_slaves_5],
          groups=["nsx_simple_stt_stop_prov"])
    @log_snapshot_on_error
    def nsx_simple_stt_stop_prov(self):
            ...
            Snapshot nsx_simple_stt_stop_prov
            ...
            self.fuel_web.security.verify_firewall(cluster_id)
            self.fuel_web.run_ostf(cluster_id=cluster_id)
            self.env.make_snapshot("nsx_simple_stt_stop_prov")

    @test(groups=["nsx"])
    @test(depends_on=[SetupEnvironment.prepare_slaves_5],
          groups=["nsx_simple_sst_restart_node"])
    @log_snapshot_on_error
    def nsx_simple_sst_restart_node(self):
    ...
    @test(groups=["nsx"])
    @test(depends_on=[SetupEnvironment.prepare_slaves_5],
          groups=["nsx_simple_gre_stop_deploy"])
    @log_snapshot_on_error
    def nsx_simple_gre_stop_deploy(self):
        ...
        Snapshot nsx_simple_gre_stop_deploy
        ...
        self.fuel_web.security.verify_firewall(cluster_id)
        self.fuel_web.run_ostf(cluster_id=cluster_id)
        self.env.make_snapshot("nsx_simple_gre_stop_deploy")

    @test(depends_on=[SetupEnvironment.prepare_slaves_3],
          groups=["nsx_simple_gre_add_one_node_conf2"])
    @log_snapshot_on_error
    def nsx_simple_gre_add_one_node_conf2(self):

        """Deploy cluster in simple mode
           with Neutron NSX plugin
           with gre in normal deploy mode,
        ...
        self.env.make_snapshot("nsx_simple_gre_add_one_node_conf2")
...
class NSX_Acceptance_HA(TestBasic):
    @test(depends_on=[SetupEnvironment.prepare_slaves_7],
          groups=["nsx_HA_stt_stop_provisioning"])
    @log_snapshot_on_error
    def nsx_HA_stt_stop_provisioning(self):
        ...
        Snapshot nsx_HA_stt_stop_provisioning
        ...
        self.fuel_web.security.verify_firewall(cluster_id)
        self.fuel_web.run_ostf(cluster_id=cluster_id)
        self.env.make_snapshot("nsx_HA_stt_stop_provisioning")

    @test(groups=["nsx"])
    @test(depends_on=[SetupEnvironment.prepare_slaves_5],
          groups=["nsx_HA_stt_reset_node"])
    @log_snapshot_on_error
    def nsx_HA_stt_reset_node(self):
            ...
            Snapshot nsx_HA_stt_reset_node
            ...
            self.fuel_web.security.verify_firewall(cluster_id)
            self.fuel_web.run_ostf(cluster_id=cluster_id)
            self.env.make_snapshot("nsx_HA_stt_reset_node")

    @test(groups=["nsx"])
    @test(depends_on=[SetupEnvironment.prepare_slaves_5],
          groups=["nsx_HA_sst_stop_deploy"])
    @log_snapshot_on_error
    def nsx_HA_sst_stop_deploy(self):
            ...
            Snapshot nsx_HA_sst_stop_deploy
            ...
            self.fuel_web.security.verify_firewall(cluster_id)
            self.fuel_web.run_ostf(cluster_id=cluster_id)
            self.env.make_snapshot("nsx_HA_sst_stop_deploy")

    @test(groups=["nsx"])
    @test(depends_on=[SetupEnvironment.prepare_slaves_5],
          groups=["nsx_HA_stt_delete_node"])
    @log_snapshot_on_error
    def nsx_HA_stt_delete_node(self):
            ...
            Snapshot nsx_HA_stt_delete_node
            ...
            self.fuel_web.security.verify_firewall(cluster_id)
            self.fuel_web.run_ostf(cluster_id=cluster_id)
            self.env.make_snapshot("nsx_HA_stt_delete_node")

    @test(groups=["nsx"])
    @test(depends_on=[SetupEnvironment.prepare_slaves_7],
          groups=["nsx_HA_sst_add_node"])
    @log_snapshot_on_error
    def nsx_HA_sst_add_node(self):
        ...
        self.fuel_web.security.verify_firewall(cluster_id)
        self.fuel_web.run_ostf(cluster_id=cluster_id)
        self.env.make_snapshot("nsx_HA_sst_add_node")

    @test(groups=["nsx"])
    @test(depends_on=[SetupEnvironment.prepare_slaves_5],
          groups=["nsx_HA_gre_reset_node"])
    @log_snapshot_on_error
    def nsx_HA_gre_reset_node(self):
            ...
            Snapshot nsx_HA_gre_reset_node
            ...
            self.fuel_web.security.verify_firewall(cluster_id)
            self.fuel_web.run_ostf(cluster_id=cluster_id)
            self.env.make_snapshot("nsx_HA_gre_reset_node")
