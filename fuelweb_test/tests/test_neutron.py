...
class NeutronGre(TestBasic):
    ...
    @test(groups=["thread_3", "neutron"], depends_on=[deploy_neutron_gre])
    @log_snapshot_on_error
...
class NeutronVlan(TestBasic):
    ...
    @test(groups=["thread_3", "neutron"], depends_on=[deploy_neutron_vlan])
    @log_snapshot_on_error
...
class NeutronGreHa(TestBasic):
    ...
    @test(groups=["thread_3", "neutron"], depends_on=[deploy_neutron_gre_ha])
    @log_snapshot_on_error
...
class NeutronVlanHa(TestBasic):
    ...
    @test(groups=["thread_3", "neutron"], depends_on=[deploy_neutron_vlan_ha])
    @log_snapshot_on_error
