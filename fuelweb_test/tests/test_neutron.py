...
@test(groups=["neutron", "ha", "ha_neutron", "image-based"])
class NeutronGreHa(TestBasic):
    ...
    def deploy_neutron_gre_ha(self):
        ...
        self.fuel_web.verify_network(cluster_id)
        devops_node = self.fuel_web.get_controller_with_running_service(
            self.env.nodes().slaves[0], "vip__public\s+"
                                        "\(ocf::mirantis:ns_IPaddr2\):\s+"
                                        "Started (node-\d+)")
