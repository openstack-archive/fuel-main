...
class MuranoSimple(TestBasic):
    ...
    @test(depends_on=[SetupEnvironment.prepare_slaves_3],
          groups=["deploy_murano_simple_vlan"])
    @log_snapshot_on_error
    def deploy_murano_simple_vlan(self):
        """Deploy cluster in simple mode with Murano and Neutron VLAN
        ...
        Snapshot: deploy_murano_simple_vlan
        ...
        self.env.make_snapshot("deploy_murano_simple_vlan")
...
class MuranoHA(TestBasic):
    ...
    @test(depends_on=[SetupEnvironment.prepare_slaves_5],
          groups=["deploy_murano_ha_with_vlan"])
    @log_snapshot_on_error
    def deploy_murano_ha_with_vlan(self):
        """Deploy cluster in ha mode with Murano and Neutron VLAN
        ...
        Snapshot: deploy_murano_ha_with_vlan
        ...
        self.env.make_snapshot("deploy_murano_ha_with_vlan")
