...
class SavannaSimple(TestBasic):
    ...
    @test(
        groups=["thread_1", "services.savanna"],
    ...
    @test(
        groups=["thread_1", "services.savanna"],
        depends_on=[deploy_savanna_simple])
    def savanna_simple_ostf(self):
...
class MuranoSimple(TestBasic):
    @log_snapshot_on_error
    @test(
        groups=["thread_1", "services.murano"],
        depends_on=[SetupEnvironment.prepare_slaves_5])
    ...
    @test(
        groups=["thread_1", "services.murano"],
        depends_on=[deploy_murano_simple])
    def murano_simple_ostf(self):
