...
class SavannaSimple(TestBasic):
    ...
    @test(depends_on=[deploy_savanna_simple_ostf],
          groups=["deploy_savanna_simple_import_image"])
    @log_snapshot_on_error
    def deploy_savanna_simple_platform_ostf(self):
        """Run OSTF platform tests on cluster with Savanna

        Scenario:
            1. Revert snapshot "deploy_savanna_simple"
            3. Register savanna image
            2. Run OSTF platform tests
