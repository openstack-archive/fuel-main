...
class TestNeutronFailover(base_test_case.TestBasic):
    ...
    def check_agents_migration(self):
        ...
        for i in range(0, 10):
            ...
            wait(lambda: not os_conn.get_node_with_dhcp_for_network(
                 net_id), interval=1, timeout=60 * 3)
            wait(lambda:
                 os_conn.get_node_with_dhcp_for_network(net_id),
                 timeout=60 * 3)

            remote.execute("pcs constraint location p_neutron-l3-agent"
                           " prefers {0}=-INFINITY".format(node_with_l3))
            wait(lambda: not os_conn.get_l3_agent_hosts(router_id),
                 interval=1,
                 timeout=60 * 3)
            wait(lambda: os_conn.get_l3_agent_hosts(router_id),
                 timeout=60 * 3)
