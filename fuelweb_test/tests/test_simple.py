...
class OneNodeDeploy(TestBasic):
    ...
    @test(groups=["thread_2"], depends_on=[SetupEnvironment.prepare_release])
...
class SimpleFlat(TestBasic):
    ...
    @test(groups=["thread_2", "smoke"], depends_on=[deploy_simple_flat])
    @log_snapshot_on_error
    ...
    def simple_flat_blocked_vlan(self):
        ...
        ebtables = self.env.get_ebtables(
            cluster_id, self.envdevops_nodes.nodes().slaves[:2])
    ...
    def simple_flat_add_compute(self):
        ...
        assert_equal(3, len(self.client.list_cluster_nodes(cluster_id)))
    ...
    @test(groups=["thread_2"], depends_on=[simple_flat_add_compute])
    @log_snapshot_on_error
...
class SimpleVlan(TestBasic):
    ...
    @test(groups=["thread_2"], depends_on=[SetupEnvironment.prepare_slaves_3])
    @log_snapshot_on_error
...
class MultiroleControllerCinder(TestBasic):
    ...
    @test(groups=["thread_3"], depends_on=[deploy_multirole_controller_cinder])
    @log_snapshot_on_error
...
class MultiroleComputeCinder(TestBasic):
    ...
    @test(groups=["thread_3"], depends_on=[deploy_multirole_compute_cinder])
    @log_snapshot_on_error
...
class UntaggedNetwork(TestBasic):
    ...
    @test(groups=["thread_1"], depends_on=[SetupEnvironment.prepare_slaves_3])
    @log_snapshot_on_error
    ...
    @test(groups=["thread_2"], depends_on=[prepare_untagged_network])
    @log_snapshot_on_error
    def untagged_network_verify_networks(self):
        self.env.revert_snapshot("prepare_untagged_network")
        self.fuel_web.verify_network(self.fuel_web.get_last_created_cluster())

    @test(groups=["thread_1"], depends_on=[prepare_untagged_network])
    @log_snapshot_on_error
    ...
    @test(groups=["thread_2"], depends_on=[deploy_untagged_network])
    @log_snapshot_on_error
...
class FloatingIPs(TestBasic):
    ...
    @test(groups=["thread_2"], depends_on=[deploy_floating_ips])
    @log_snapshot_on_error
...
class SimpleCinder(TestBasic):
    ...
    @test(groups=["thread_1"], depends_on=[deploy_simple_cinder])
    @log_snapshot_on_error
...
class NodeMultipleInterfaces(TestBasic):
    ...
    @test(groups=["thread_1"], depends_on=[deploy_node_multiple_interfaces])
    @log_snapshot_on_error
...
class NodeDiskSizes(TestBasic):
    ...
    @test(groups=["thread_1"], depends_on=[SimpleCinder.deploy_simple_cinder])
    @log_snapshot_on_error
    ...
    def multinic_bootstrap_booting(self):
        ...
        try:
            ...
            for mac in mac_addresses:
                ...
                self.nodes().admins[0].revert("ready")
