...
from proboscis import asserts, SkipTest
...
class EnvironmentAction(base_test_case.TestBasic):
    ...
    def deploy_flat_stop_reset_on_provisioning(self):
        ...
        try:
            ...
            if e.code == 400 and 'Nothing to stop' in e.read():
                raise SkipTest('Stop deployment on granular '
    ...
    def deploy_stop_reset_on_ha(self):
        ...
        self.fuel_web.deploy_cluster_wait(cluster_id, progress=20)
        self.fuel_web.stop_deployment_wait(cluster_id)
        self.fuel_web.stop_reset_env_wait(cluster_id)
        self.fuel_web.update_nodes(
            cluster_id,
