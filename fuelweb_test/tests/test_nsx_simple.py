...
class NsxTestsInSimpleMode(TestBasic):
    ...
    @test(depends_on=[SetupEnvironment.prepare_slaves_3],
          groups=["deploy_simple_nsx_ipsec_gre"])
    @log_snapshot_on_error
    def deploy_simple_nsx_ipsec_gre(self):
        """
        Deploy cluster in simple mode,
           with Neutron NSX plugin,
           with GRE over IPSec,
        ...
        self.fuel_web.verify_network(cluster_id)
        self.fuel_web.run_ostf(cluster_id=cluster_id)
        self.env.make_snapshot("deploy_simple_nsx_ipsec_gre")

    @test(depends_on=[SetupEnvironment.prepare_slaves_3],
          groups=["deploy_simple_nsx_ipsec_stt"])
    @log_snapshot_on_error
    def deploy_simple_nsx_ipsec_stt(self):
        """
        Deploy cluster in simple mode,
           with Neutron NSX plugin,
           with STT over IPSec,
        ...
        self.fuel_web.verify_network(cluster_id)
        self.fuel_web.run_ostf(cluster_id=cluster_id)
        self.env.make_snapshot("deploy_simple_nsx_ipsec_stt")

    @test(depends_on=[SetupEnvironment.prepare_slaves_3],
          groups=["deploy_simple_nsx_bridge"])
    @log_snapshot_on_error
    def deploy_simple_nsx_bridge(self):
        """
        Deploy cluster in simple mode,
           with Neutron NSX plugin,
           with Bridge,
        ...
        self.fuel_web.verify_network(cluster_id)
        self.fuel_web.run_ostf(cluster_id=cluster_id)
        self.env.make_snapshot("deploy_simple_nsx_bridge")