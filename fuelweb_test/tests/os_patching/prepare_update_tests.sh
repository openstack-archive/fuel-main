...
archive_url=" http://srv11-msk.msk.mirantis.net/cloud-images"
...
if [ ! -f "$centos_repo_archive" ]; then
	wget -O $centos_repo_archive $archive_url/$centos_repo_archive
fi

if [ ! -f "$ubuntu_repo_archive" ]; then
        wget -O $ubuntu_repo_archive $archive_url/$ubuntu_repo_archive
fi
