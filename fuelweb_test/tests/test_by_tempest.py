...
def safe_revert_snapshot(env, snapshot_name):
    success = env.revert_snapshot(snapshot_name)
...
@test(groups=["tempest"])
class TempestSet(base_test_case.TestBasic):
    ...
    def run_tempest(self):
        ...
        safe_revert_snapshot(self.env, os.environ.get("SNAPSHOT"))
...
@test(groups=["tempest"])
class TempestList(base_test_case.TestBasic):
