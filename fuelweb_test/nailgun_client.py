...
class NailgunClient(object):
    ...
    def get_networks(self, cluster_id):
        return self.client.get("/api/clusters/%d/network_configuration/" % cluster_id)

    @logwrap
    @json_parse
    def verify_networks(self, cluster_id, networks):
        return self.client.put(
            "/clusters/%d/network_configuration/verify/" % cluster_id, {'networks': networks}
        )
    ...
    def update_network(self, cluster_id, flat_net):
        return self.client.put(
            "/api/clusters/%d/network_configuration" % cluster_id, {'networks': flat_net}
        )
    ...
    def _get_cluster_vlans(self, cluster_id):
        cluster_vlans = []
        for network in self.get_networks(cluster_id):
            amount = network['network'].get('amount', 1)
            cluster_vlans.extend(range(network['network']['vlan_start'],
                                       network['network']['vlan_start'] + amount))
