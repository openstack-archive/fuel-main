...
ACTIONS='is-active|is-enabled|start|stop|reload|restart|status'

function usage() {
    echo "* Usage: $(basename ${0}) ${ACTIONS} service_name"
}
...
if [ "${service_name}" == 'supervisord' ]; then
  case ${action} in
    'is-active')
      /etc/init.d/supervisord.init status || res=$?
    ;;
    'is-enabled')
    ;;
    *)
      /etc/init.d/supervisord.init ${action} || res=$?
    ;;
  esac
  exit $res
fi
...
case ${action} in
  ...
  ;;
  'is-enabled')
    value=$(iniget "${SUPERVISORD_CONF}" "program:${service_name}" "enabled")
    [[ "${value}" =~ [Tt]rue ]] || res=1
  ;;
  *)
...
#ln -nsf /bin/true /usr/bin/systemctl
