...
ACTIONS['systemctl']='enable|disable|is-active|is-enabled|start|stop|reload|restart|status'
...
case ${action} in
  'enable')
    iniset "${SUPERVISORD_CONF}" "program:${service_name}" "autostart" "true"
    for conf_file in $(find ${SUPERVISORD_CONFDIR} -type f); do
      iniset "${conf_file}" "program:${service_name}" "autostart" "true"
    done
  ;;
  'disable')
    iniset "${SUPERVISORD_CONF}" "program:${service_name}" "autostart" "false"
    for conf_file in $(find ${SUPERVISORD_CONFDIR} -type f); do
      iniset "${conf_file}" "program:${service_name}" "autostart" "false"
    done
  ;;
  'is-active')
...
sed -i 's|^files.*|files = /etc/supervisord.d/*.conf|g' /etc/supervisord.conf
