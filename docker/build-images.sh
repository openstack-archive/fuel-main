...
for container in $containers; do
  ...
  #dockerref=refs/changes/61/83061/4
  #puppetrepo=git://repo.git
  #puppetref=$(cat $container/puppet)

  rm -rf "$BUILD_DIR/docker/$container"
  #mkdir -p "$BUILD_DIR/docker/$container"
  #git init docker-$container
  git clone --depth 1 --branch $dockerref $dockerrepo $BUILD_DIR/docker/$container
  #pushd "$BUILD_DIR/docker/$container"
  #git checkout $dockerref
  #popd
  if [ "$puppetref" != "none" -a "$puppetref" != "same" ]; then
    rm -rf $BUILD_DIR/docker/puppet
    if grep -q "refs/changes" <<< "$puppetref"; then
      git clone --depth 1 --branch master $puppetrepo $BUILD_DIR/docker/puppet
    ...
    #git init puppet
    #git remote add puppet $puppetrepo
    #git fetch puppet $puppetref
    #git clone $puppetrepo .
    #git checkout $puppetref
  ...
  docker build -rm -t fuel/$container $container
  ...
  docker export $container_id | gzip -c > "${BUILD_DIR}/isoroot/docker/${container}.tar.gz"
  popd
done
