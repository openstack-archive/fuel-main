...
$(BUILD_DIR)/docker/build.done: \
		...
		$(BUILD_DIR)/docker/fuel-centos-build.done \
		$(BUILD_DIR)/docker/sources.done
...
$(BUILD_DIR)/docker/fuel-centos.done: \
		$(BUILD_DIR)/docker/base-images.done
	(cd $(LOCAL_MIRROR_CENTOS) && python $(SOURCE_DIR)/utils/simple_http_daemon.py $(RANDOM_PORT) /tmp/simple_http_daemon_$(RANDOM_PORT).pid)
	rm -rf $(BUILD_DIR)/docker/centos
	cp -a $(SOURCE_DIR)/docker/fuel-centos-build $(BUILD_DIR)/docker/centos
	sed -e "s/_PORT_/$(RANDOM_PORT)/" -i $(BUILD_DIR)/docker/centos/Dockerfile
	sudo docker build -t fuel/fuel-centos-build_$(PRODUCT_VERSION) $(BUILD_DIR)/docker/fuel-centos-build
	mkdir -p "$(BUILD_DIR)/docker/centos/output"
	sudo docker run -n fuel-centos-build -i -t --privileged -v $(LOCAL_MIRROR_CENTOS):/repo:ro -v $(BUILD_DIR)/docker/centos/output:/export fuel/fuel-centos-build
