...
REPO_CONTAINER:=fuel-repo-container${BUILD_NUMBER}
...
$(BUILD_DIR)/docker/repo-container-up.done: \
		$(BUILD_DIR)/docker/fuel-centos.done
	sudo docker -D run -d -p 80 -v $(LOCAL_MIRROR_CENTOS):/var/www/html -n "$(REPO_CONTAINER)" fuel/centos /usr/sbin/apachectl -DFOREGROUND
	sleep 5
...
mmosesohn@fueliso:~/fuel-main$ 
mmosesohn@fueliso:~/fuel-main$ cat docker/module.mk
.PHONY: docker
containers:=astute cobbler mcollective nailgun keystone nginx ostf rsync rsyslog rabbitmq postgres
REPO_CONTAINER:=fuel-repo-container${BUILD_NUMBER}
