...
$(BUILD_DIR)/docker/$1.done: \
	...
	sudo docker exec -i FUEL_$1_$(PRODUCT_VERSION) /usr/local/bin/setup.sh 2>&1 \
		|| touch $(BUILD_DIR)/docker/$1.failure
	@test -f $(BUILD_DIR)/docker/$1.failure \
		&& echo "BUILD FAILED, container '$1'. Ignoring ..." \
		|| echo "BUILD SUCCEEDED, container '$1'."
	@echo "========================================"
	@echo ""
	@test -f $(BUILD_DIR)/docker/$1.failure \
		&& bash $(SOURCE_DIR)/docker/debug.sh \
		||:
	$$(ACTION.TOUCH)
endef
