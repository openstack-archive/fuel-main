...
$(BUILD_DIR)/docker/$1.done: \
	...
	sudo docker run --name=FUEL_$1_$(PRODUCT_VERSION) --net=bridge -d -i -t --privileged fuel/$1_$(PRODUCT_VERSION)
	sudo docker exec -i FUEL_$1_$(PRODUCT_VERSION) /usr/local/bin/setup.sh 2>&1 || touch $(BUILD_DIR)/docker/$1.failed
	test -f $(BUILD_DIR)/docker/$1.failed && echo "BUILD FAILED, container '$1'. Ignoring ..." && rm -f $(BUILD_DIR)/docker/$1.failed
	test -f $(BUILD_DIR)/docker/$1.failed || echo "BUILD SUCCEEDED, container '$1'"
	echo "========================================"
	echo ""
	$$(ACTION.TOUCH)
endef
...
$(BUILD_DIR)/docker/fuel-centos.done: \
	...
	sudo docker -D run --name=FUEL_CENTOS_$(PRODUCT_VERSION) --net=bridge -d -i -t --privileged \
		-v $(LOCAL_MIRROR_CENTOS):/repo:ro -v $(BUILD_DIR)/docker/fuel-centos:/export fuel/fuel-centos-build
