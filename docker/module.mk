...
$(BUILD_DIR)/docker/fuel-centos.done: \
	...
	sudo docker -D run --name=FUEL_CENTOS_$(PRODUCT_VERSION) --net=bridge -d -i -t --privileged \
		-v $(LOCAL_MIRROR_CENTOS):/repo:ro -v $(LOCAL_MIRROR_MOS_CENTOS):/mos-repo:ro -v $(BUILD_DIR)/docker/fuel-centos:/export fuel/fuel-centos-build
...
$(BUILD_DIR)/docker/repo-container-up.done: \
	...
	sudo docker -D run --name "$(REPO_CONTAINER)" -d -p 80 \
		-v $(LOCAL_MIRROR_CENTOS):/var/www/html/repo -v $(LOCAL_MIRROR_MOS_CENTOS):/var/www/html/mos-repofuel/centos /usr/sbin/apachectl -DFOREGROUND
	REPO_PORT=`sudo docker port $(REPO_CONTAINER) 80 | cut -d':' -f2` && \
