...
from fuelweb_test.testrail import APIClient
...
class TestRailProject():
    ...
    def get_cases(self, suite):
        cases_uri = 'get_cases/{project_id}&suite_id={suite_id}'.format(
            project_id=self.project['id'], suite_id=self.get_suite(suite)['id']
        )
        return self.client.send_get(cases_uri)

    def get_case_by_name(self, suite, name):
        for case in self.get_cases(suite):
            if case['title'] == name:
                return case

    def get_case_by_group(self, suite, group):
        for case in self.get_cases(suite):
    ...
    def get_test_by_group(self, run_id, group):
        for test in self.get_tests(run_id):
    ...
    def add_results_for_test(self, test_id, test_results):
        ...
        new_results = {
            ...
            'version': '_'.join([test_results.version] +
                                test_results.description.split())
        }
