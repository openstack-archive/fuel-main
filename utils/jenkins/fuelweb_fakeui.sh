...
sed 's|^TEMPLATE_DIR:.*$|'TEMPLATE_DIR: "$STATIC_DIR"'|' -i $WORKSPACE/build/repos/nailgun/nailgun/settings.yaml
sed 's|^STATIC_DIR:.*$|'STATIC_DIR: "$STATIC_DIR"'|' -i $WORKSPACE/build/repos/nailgun/nailgun/settings.yaml
sed 's|^DEVELOPMENT:.*$|'DEVELOPMENT: false|' -i $WORKSPACE/build/repos/nailgun/nailgun/settings.yaml

# Show date and commit hash in ui
VERSION_TEXT=`git show -s --format=%ci HEAD` `git rev-parse --verify HEAD`
sed 's|Unknown version|'"$VERSION_TEXT"'|' -i $WORKSPACE/build/repos/nailgun/nailgun/settings.yaml
