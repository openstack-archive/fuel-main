...
def get_license(package, stop='no'):
    link = urllib.urlopen('%s%s' % ('https://registry.npmjs.org/', package))
    if link.getcode() == 404:
        link = urllib.urlopen('%s%s' % ('https://registry.npmjs.org/', package.split('/')[0]))
    data = json.load(link)
    ...
    if 'versions' in data.keys():
        if stop == 'yes':
            return 'Unknown'
        return get_license('%s/%s' % (package, data['versions'].keys()[0]), stop='yes')
    return 'Unknown'
        
def get_copyright(package, stop='no'):
    link = urllib.urlopen('%s%s' % ('https://registry.npmjs.org/', package))
    if link.getcode() == 404:
        link = urllib.urlopen('%s%s' % ('https://registry.npmjs.org/', package.split('/')[0]))
    data = json.load(link)
    ...
    if 'repository' in data.keys():
        if 'url' in data['repository']:
            if '/github.com' in data['repository']['url']:
                ...
                link_vars = ['/master/LICENSE', '/master/LICENSE.txt', '/master/license', '/master/license.txt', '/master/LICENSE.md', '/master/license.md']
                for name in link_vars:
                    link_name = link + name
                    licenselink = urllib.urlopen(github_parseurl(link_name))
                    copyright = check_url(licenselink)
                    if copyright:
                        return copyright
    if 'versions' in data.keys():
        if stop == 'yes':
            return ''
        return get_copyright('%s/%s' % (package, data['versions'].keys()[0]), stop='yes')
    return ''
