...
# packages
SANDBOX_PACKAGES="@buildsys-build sudo yum yum-utils"

# centos mirror
MIRROR=${CENTOS_MIRROR:-http://mirror.fuel-infra.org/mos-repos/centos/mos${PRODUCT_VERSION}-centos7-fuel/os/x86_64/}
...
SANDBOX_MIRROR_CENTOS_UPSTREAM="http://mirror.centos.org/centos-7/${CENTOS_RELEASE}"
...
reposdir=/etc/yum.repos.d/wrong_place
...
sudo curl -sSf https://raw.githubusercontent.com/openstack/fuel-main/master/mirror/centos/yum-priorities-plugin.py -o \
...
sudo /bin/sh -c "export TMPDIR=${rootfsDir}/tmp/yum TMP=${rootfsDir}/tmp/yum ; \
  ...
  yum -c ${rootfsDir}/etc/yum.conf --installroot=${rootfsDir} clean all"
