...
$(BUILD_DIR)/bootstrap/prepare-initram-root.done: \
	...
	sudo chroot $(INITRAMROOT) sh -c "rpm --dbpath /root/.rpmdb -qa >> /bootstrap_packages"
	sudo for i in `find $(LOCAL_MIRROR_CENTOS_OS_BASEURL) -name '$(KERNEL_PATTERN)' `  ; do basename ${i} >> $(INITRAMROOT)/bootstrap_kernel ; done
	sudo for i in `find $(LOCAL_MIRROR_CENTOS_OS_BASEURL) -name '$(KERNEL_FIRMWARE_PATTERN)' `  ; do basename ${i} >> $(INITRAMROOT)/bootstrap_kernel ; done
