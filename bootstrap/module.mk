...
$(BUILD_DIR)/bootstrap/linux: $(BUILD_DIR)/mirror/centos/build.done
	...
	find $(LOCAL_MIRROR_CENTOS) -name '$(KERNEL_PATTERN)' | xargs rpm2cpio | \
...
# Perhaps this stuff should be moved to global config.mk
	...
	find $(LOCAL_MIRROR_CENTOS) -name '$(KERNEL_FIRMWARE_PATTERN)' | xargs rpm2cpio | \
		( cd $(INITRAMROOT); sudo cpio $(CPIOARGS) -idm './lib/firmware/*' )
	find $(LOCAL_MIRROR_CENTOS) -name 'libmlx4*' | xargs rpm2cpio | \
