...
$(BUILD_DIR)/centos_mirror.yaml.mk: $(CENTOS_MIRROR_YAML)
	mkdir -p $(@D)
	python $(SOURCE_DIR)/evaluator.py \
		-m -c $(CENTOS_MIRROR_YAML) \
		-p $(notdir $(CENTOS_MIRROR_YAML))_ -o $@

$(BUILD_DIR)/ubuntu_mirror.yaml.mk: $(UBUNTU_MIRROR_YAML)
	mkdir -p $(@D)
	python $(SOURCE_DIR)/evaluator.py \
		-m -c $(UBUNTU_MIRROR_YAML) \
		-p $(notdir $(UBUNTU_MIRROR_YAML))_ -o $@
...
include $(BUILD_DIR)/centos_mirror.yaml.mk
include $(BUILD_DIR)/ubuntu_mirror.yaml.mk
