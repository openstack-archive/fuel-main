...
ifneq ($(BUILD_ARTIFACTS),0)
$(BUILD_DIR)/upgrade/openstack-part.tar: $(BUILD_DIR)/iso/iso.done
endif
...
os_version:=$(shell python -c "import yaml; print filter(lambda r: r['fields'].get('name'), yaml.load(open('$(BUILD_DIR)/repos/nailgun/nailgun/nailgun/fixtures/openstack.yaml')))[0]['fields']['version']")

$(BUILD_DIR)/upgrade/openstack-part.tar:
	mkdir -p $(@D)
	rm -f $@
ifneq ($(BUILD_ARTIFACTS),0)
	tar cf $@ -C $(BUILD_DIR)/repos/fuellib/deployment --xform s:^puppet:upgrade/puppet/$(os_version)/modules: puppet
	tar rf $@ -C $(BUILD_DIR)/repos/fuellib/deployment/puppet/osnailyfacter/examples --xform s:^:upgrade/puppet/$(os_version)/manifests/: site.pp
	tar cf $@ -C $(LOCAL_MIRROR) --xform s:^centos/os/x86_64:upgrade/repos/$(os_version)/centos/x86_64: centos/os/x86_64
	tar rf $@ -C $(LOCAL_MIRROR) --xform s:^ubuntu:upgrade/repos/$(os_version)/ubuntu/x86_64: ubuntu
	tar rf $@ -C $(BUILD_DIR)/repos/nailgun/nailgun/nailgun/fixtures/openstack.yaml --xform s:^:upgrade/releases/$(os_version): openstack.yaml
	tar rf $@ -C $(ISOROOT) --xform s:^:upgrade/puppet/$(os_version)/manifests/: centos-versions.yaml ubuntu-versions.yaml
