...
class Provision:
    def __init__(self):
        raise NotImplementedError("Try to use ProvisionFactory.getInstance() method.")
...
class ProvisionFactory:
    ...
    def getInstance(cls, config=ProvisionConfig()):
        ...
        return getattr(__import__(module_name, globals(), locals(), [class_name], -1), class_name)(config)
