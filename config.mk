...
expand_repo_url=$(shell $(expand_repo_script))
define expand_repo_script
url=$(1);
if echo $${url} | grep -q -e '\.target\.txt$$';
then
  echo "$${url%/*}/$$(curl -sSf $$url | head -1)/x86_64/";
else
  echo $${url};
fi
endef
...
override MIRROR_FUEL:=$(call expand_repo_url,$(MIRROR_FUEL))
