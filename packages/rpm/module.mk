...
$(BUILD_DIR)/packages/rpm/$1.done:
	...
	test -f $$(SANDBOX)/tmp/SOURCES/version && \
		...
		sudo chroot $$(SANDBOX) rpmbuild --nodeps --define "_topdir /tmp" -ba /tmp/$1.spec --define "release $$$$(( `LANG=C yum -c $$(SANDBOX)/etc/yum.conf --installroot=$$(SANDBOX) --disablerepo=* --enablerepo=mirror info $1 | awk -F ':' '/Release/ {print $$$$2}' | tr -d ' '` + 1 ))"
...
$(BUILD_DIR)/packages/rpm/fuel-docker-images.done: \
	...
	sudo chroot $(SANDBOX) rpmbuild --nodeps --define "release $$$$(( `LANG=C yum -c $$(SANDBOX)/etc/yum.conf --installroot=$$(SANDBOX) --disablerepo=* --enablerepo=mirror info $1 | awk -F ':' '/Release/ {print $$$$2}' | tr -d ' '` + 1 ))" --define "_topdir /tmp" -ba /tmp/fuel-docker-images.spec
