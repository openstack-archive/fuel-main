...
$(BUILD_DIR)/packages/rpm/rpm-fuel-cli.done: SANDBOX:=$(BUILD_DIR)/packages/rpm/SANDBOX
$(BUILD_DIR)/packages/rpm/rpm-fuel-cli.done: export SANDBOX_UP:=$(SANDBOX_UP)
$(BUILD_DIR)/packages/rpm/rpm-fuel-cli.done: export SANDBOX_DOWN:=$(SANDBOX_DOWN)
$(BUILD_DIR)/packages/rpm/rpm-fuel-cli.done: $(BUILD_DIR)/repos/nailgun.done
$(BUILD_DIR)/packages/rpm/rpm-fuel-cli.done: \
		$(BUILD_DIR)/packages/rpm/prep.done \
		$(SOURCE_DIR)/packages/rpm/specs/fuel-cli.spec \
		$(call find-files,$(BUILD_DIR)/repos/nailgun/fuel-cli)
	sudo sh -c "$${SANDBOX_UP}"
	sudo mkdir -p $(SANDBOX)/tmp/SOURCES/fuel-cli
	sudo cp -r $(BUILD_DIR)/repos/nailgun/fuel-cli/* $(SANDBOX)/tmp/SOURCES/fuel-cli
	cd $(SANDBOX)/tmp/SOURCES/fuel-cli && sudo python setup.py sdist -d $(SANDBOX)/tmp/SOURCES
	sudo cp $(SOURCE_DIR)/packages/rpm/specs/fuel-cli.spec $(SANDBOX)/tmp
	sudo chroot $(SANDBOX) rpmbuild -vv --define "_topdir /tmp" -ba /tmp/fuel-cli.spec
	cp $(SANDBOX)/tmp/RPMS/noarch/fuel-cli-*.rpm $(BUILD_DIR)/packages/rpm/RPMS/x86_64/
	sudo sh -c "$${SANDBOX_DOWN}"
	$(ACTION.TOUCH)
