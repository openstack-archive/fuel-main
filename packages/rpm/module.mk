...
$(BUILD_DIR)/packages/rpm/fuel-docker-images.done: \
	...
	sudo chroot $(SANDBOX) rpmbuild --nodeps --define "release $$$$(( `LANG=C yum -c $(SANDBOX)/etc/yum.conf --installroot=$(SANDBOX) --disablerepo=* --enablerepo=mirror info $1 | awk -F ':' '/Release/ {print $$$$2}' | tr -d ' ' | sort -n | tail -n 1` + 1 ))" --define "_topdir /tmp" -ba /tmp/fuel-docker-images.spec
