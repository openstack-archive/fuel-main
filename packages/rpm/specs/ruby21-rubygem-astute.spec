...
%files
%defattr(-, root, root)
%{gemdir}/gems/%{name}-%{version}/bin/*
%{gemdir}/gems/%{name}-%{version}/lib/*
%{gemdir}/gems/%{name}-%{version}/spec/*

%dir %attr(0750, naily, naily) %{_sysconfdir}/%{name}
%config(noreplace) %attr(0640, root, naily) %{_sysconfdir}/%{name}/astute.conf
%dir %attr(0755, naily, naily) %{_localstatedir}/log/%{name}
