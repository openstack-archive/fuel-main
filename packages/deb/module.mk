...
$(BUILD_DIR)/packages/deb/$1.done: $(BUILD_DIR)/repos/repos.done
	...
	DEBFULLNAME=`grep DEBFULLNAME $(BUILD_DIR)/packages/sources/$1/version | cut -d'=' -f2` \
		DEBEMAIL=`grep DEBEMAIL $(BUILD_DIR)/packages/sources/$1/version | cut -d'=' -f2` \
		sudo -E dch -c $$(SANDBOX_UBUNTU)/tmp/$1/debian/changelog -b --force-distribution \
		-v $(PACKAGE_VERSION)-`head -1 $(BUILD_DIR)/packages/sources/$1/version` \
		`grep DEBMSG $(BUILD_DIR)/packages/sources/$1/version | cut -d'=' -f2`
