...
# Also prepare version file in format:
# if gerrit commit is given then
#   ${commit_num}.0.gerrit${request_number}.${patchset_number}.git${git_sha}
# else
#   ${commit_num}.1.git${git_sha}
...
$(BUILD_DIR)/packages/sources/$1/$2:
	...
	echo ".git`cd $3 && git rev-parse --short $4`" >> $$(VERSIONFILE)
	echo DEBFULLNAME=`git -C $(BUILD_DIR)/repos/$1 log -1 --pretty=format:%an` >> $$(VERSIONFILE)
	echo DEBEMAIL=`git -C $(BUILD_DIR)/repos/$1 log -1 --pretty=format:%ae` >> $$(VERSIONFILE)
	echo DEBMSG=`git -C $(BUILD_DIR)/repos/$1 log -1 --pretty=%s` >> $$(VERSIONFILE)
