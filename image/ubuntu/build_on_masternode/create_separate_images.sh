...
function cleanup {
    ...
    for idx in $(seq $((${#MOUNTPOINTS[@]} - 1)) -1 0); do
        ...
        if [ -e "$LOOP_DEV" ]; then
            ...
            sudo rm -f $LOOP_DEV || die_ret "Couldn't remove old loop-device file"
        fi
        if [ -e "$SPARSE_FILE_NAME" ]; then
            sudo rm -f ${SPARSE_FILE_NAME} || die_ret "Could remove old sparce image"
        fi
    done
    return 0
}
...
function create_loop_device_and_makefs {
    for idx in $(seq 0 $((${#MOUNTPOINTS[@]} - 1)) ); do
        ...
        sudo losetup ${LOOP_DEV} ${SPARSE_FILE_NAME} || die_ret "Couldn't associate loop-device file"

        FS_TYPE=${MOUNT_DICT[$MOUNT_POINT]}
        if [ "$FS_TYPE" == "ext2" ]; then
            sudo mkfs.ext2 -F ${LOOP_DEV} || die_ret "Couldn't create filesystem"
        elif [ "$FS_TYPE" == "ext3" ]; then
            sudo mkfs.ext3 -F ${LOOP_DEV} || die_ret "Couldn't create filesystem"
        elif [ "$FS_TYPE" == "ext4" ]; then
            sudo mkfs.ext4 -F ${LOOP_DEV} || die_ret "Couldn't create filesystem"
        else
...
function do_mounts {
    for idx in $(seq 0 $((${#MOUNTPOINTS[@]} - 1)) ); do
        MOUNT_POINT=${MOUNTPOINTS[$idx]}
        LOOP_DEV=/dev/${FUEL_DEVICE_PREFIX}${idx}
        sudo mkdir -p ${TMP_CHROOT_DIR}${MOUNT_POINT} || die_ret "Could create directory"
        sudo mount ${LOOP_DEV} ${TMP_CHROOT_DIR}${MOUNT_POINT} || die_ret "Couldn't mount mountpoint"
    done
    return 0
}
...
function debootstap_download_packages_try_harder {
    ...
    while true; do
        ...
        #FIXME(MIRROR)
        sudo debootstrap --download-only --verbose $DEBOOTSTRAP_PARAMS $BASE_MIRROR_URL
        if [ $? -ne 0 ]; then
            sleep 1
            attempt=$((attempt+1))
        else
...
function install_base_system {
    ...
    # install base system
    sudo debootstrap $DEBOOTSTRAP_PARAMS $BASE_MIRROR_URL || die_ret "Couldn't finish debootstrap successfully"
}
...
function setup_apt_mirrors {
    sudo rm -f ${TMP_CHROOT_DIR}/etc/apt/sources.list
    sudo rm -f ${TMP_CHROOT_DIR}/etc/apt/preferences
    for ent in $(echo "$UBUNTU_MIRRORS"| tr ',' '\n'); do
        # expecting 'uri:suite:section:priority'
        ...
        sudo /bin/sh -c "echo deb ${uri} ${suite} ${section} >> ${TMP_CHROOT_DIR}/etc/apt/sources.list"
        sudo /bin/sh -c "echo \"Package: *\" >> ${TMP_CHROOT_DIR}/etc/apt/preferences"
        sudo /bin/sh -c "echo \"Pin: origin \"${host}\"\" >> ${TMP_CHROOT_DIR}/etc/apt/preferences"
        sudo /bin/sh -c "echo \"Pin: release n=${suite}\" >> ${TMP_CHROOT_DIR}/etc/apt/preferences"
        sudo /bin/sh -c "echo \"Pin: release a=${section}\" >> ${TMP_CHROOT_DIR}/etc/apt/preferences"
        sudo /bin/sh -c "echo \"Pin-Priority: ${priority}\" >> ${TMP_CHROOT_DIR}/etc/apt/preferences"
        sudo /bin/sh -c "echo >> ${TMP_CHROOT_DIR}/etc/apt/preferences"
    done
    return 0
}
...
function install_by_apt {
    ...
    sudo chroot ${TMP_CHROOT_DIR} apt-get update || die_ret "Couldn't update packages list from sources"
    if ! mountpoint -q ${TMP_CHROOT_DIR}/proc; then
...
function do_post_inst {
    ...
    # re-enable services
    sudo rm -f ${TMP_CHROOT_DIR}/usr/sbin/policy-rc.d
    # clean apt settings
    sudo rm -f ${TMP_CHROOT_DIR}/etc/apt/sources.list
    sudo rm -f ${TMP_CHROOT_DIR}/etc/apt/preferences
}
...
function umount_resize_images {
    for idx in $(seq $((${#MOUNTPOINTS[@]} - 1)) -1 0); do
        ...
        if [ "$FS_TYPE" == "ext2" ] || [ "$FS_TYPE" == "ext3" ] || [ "$FS_TYPE" == "ext4" ]; then
            ...
            sudo dd if=${LOOP_DEV} of=${IMG_FILE_NAME} bs=1M count=${EXPECTED_M_SIZE} || die_ret "Couldn't copy image"
    #        sudo losetup -d ${LOOP_DEV} || die_ret "Couldn't dissociate loop-device file"
    #        sudo rm -f ${LOOP_DEV} || die_ret "Couldn't remove loop-device file"
        else
            echo "Unsupported fs type $FS_TYPE. Exitting now!"
            exit 1
        fi
        sudo chmod a+r ${IMG_FILE_NAME} || die_ret "Couldn't grant reading access to image file"
        sudo gzip -f ${IMG_FILE_NAME}
        sudo mv ${IMG_FILE_NAME}.gz ${DST_DIR}/${FINAL_IMAGE_NAME}
