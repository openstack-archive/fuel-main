...
function allocate_loop_device {
    local sparse_file="$1"
    local biggest_existing_loop_idx=$(ls -v1 /dev/loop* | tail -n2 | head -n1 | cut -c 10-)
    ...
    local loop_dev=/dev/${FUEL_DEVICE_PREFIX}${biggest_existing_loop_idx}
    if [ ! -e "${loop_dev}" ]; then
...
function umount_resize_images {
    for idx in $(seq $((${#MOUNTPOINTS[@]} - 1)) -1 0); do
        ...
        if [ "$FS_TYPE" == "ext2" ] || [ "$FS_TYPE" == "ext3" ] || [ "$FS_TYPE" == "ext4" ]; then
            e2fsck -yf ${LOOP_DEV} || die "Couldn't check filesystem"
            resize2fs -F -M ${LOOP_DEV} || die "Couldn't resize filesystem"
