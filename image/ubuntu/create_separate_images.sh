...
sudo debootstrap $DEBOOTSRAP_PARAMS || die "Couldn't finish debootstrap successfully"
...
sudo chroot ${TMP_CHROOT_DIR} apt-get clean
sudo chroot ${TMP_CHROOT_DIR} mv /var/lib/apt/lists /var/lib/apt/lists.old
sudo chroot ${TMP_CHROOT_DIR} mkdir -p /var/lib/apt/lists/partial
sudo chroot ${TMP_CHROOT_DIR} apt-get clean
###
sudo chroot ${TMP_CHROOT_DIR} apt-get -q -y --force-yes update 
sudo chroot ${TMP_CHROOT_DIR} apt-get -q -y --force-yes install cloud-init
#cloud-init reconfigure to use NoCloud data source
sudo chroot ${TMP_CHROOT_DIR} echo "cloud-init cloud-init/datasources multiselect NoCloud, None" | sudo chroot ${TMP_CHROOT_DIR} debconf-set-selections -v
