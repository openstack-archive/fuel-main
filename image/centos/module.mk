...
$(BUILD_DIR)/images/$(TARGET_CENTOS_IMG_ART_NAME): $(call find-files,$(BUILD_DIR)/image/centos/$(TARGET_CENTOS_IMG_ART_NAME))
	@mkdir -p $(@D)
	...
	sudo chroot $(SANDBOX) python build_centos_image.py -k centos.ks -n centos
	sudo mv $(SANDBOX)/centos.img $(BUILD_DIR)/image/centos/
	sudo gzip -f $(BUILD_DIR)/image/centos/centos.img
	sudo sh -c "$${SANDBOX_DOWN}"
	tar czf $@ -C $(BUILD_DIR)/image centos --exclude centos/SANDBOX
endif
