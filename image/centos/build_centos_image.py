...
""" % {'image_filesystem': self._fstype,
    ...
    def _yaml_data(self):
        ...
        data = []
        guessed_profile_name = None
        for disk in self.__separate_images_disks:
            filename = os.path.basename(disk.disk.lofile)
            abs_path = self._outdir + "/" + filename
            size = os.path.getsize(abs_path)
            md5 = md5sum(abs_path)
            mountpoint = filename[len(self.name):-4].replace('-', '/')
            if not mountpoint:
                mountpoint = '/'
                guessed_profile_name = filename + '.yaml'
            data.append({mountpoint: {'filename': filename + '.gz',
                                      'size': size, 'md5': md5,
                                      'container': 'gzip',
                                      'format': disk.fstype}})
        with open(os.path.join(os.path.dirname(self.__output_file),
                               guessed_profile_name), 'wb') as f:
