...
build_ubuntu_bootstrap () {
	...
	local config='/etc/fuel/fuel-bootstrap-image-build.yaml'
...
#		fi
#	done
############# In the awesome future it will be:
#		echo "Bulding bootstrap image " >&2
#                fuel-bootstrap build --activate --config /etc/fuel-agent/fuel_bootstrap_cli.yaml --lot-to ${log} --block-of-fail
#
############# But for now...
		echo "Bulding bootstrap image " >&2
	for n in `seq 1 $max_attempts`; do
		echo "Bulding bootstrap image, attempt $n" >&2
		if fuel-bootstrap build --config /etc/fuel-agent/fuel_bootstrap_cli.yaml >>"$log" 2>&1; then
                        fuel-bootstrap-set 'ubuntu' 
			ret=0
			break
		fi
	done
        if [ $ret -ne 0 ]; then
                warning="ZZZZZ! Triggered!Will be removed at all-moved to blocker  WARNING: failed to build the bootstrap image, see $log for details.
