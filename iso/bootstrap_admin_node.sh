...
function ifdown_ethernet_interfaces {
    ...
    adminif_ipaddr=$(ip -4 -o a s ${ADMIN_INTERFACE} | sed 's:/:\ :;s:\s\+:\ :g' | cut -d ' ' -f 4)
    if [[ -z "${adminif_ipaddr}" ]]; then
        return
    fi
    for if_name in $(ip -4 -o a | sed 's:/:\ :;s:\s\+:\ :g;/\slo\s/d' | cut -d ' ' -f 2); do
        if [[ "${if_name}" == "${ADMIN_INTERFACE}" ]]; then
            continue
        fi
        if_ipaddr=$(ip -4 -o a s ${if_name} | sed 's:/:\ :;s:\s\+:\ :g' | cut -d ' ' -f 4)
