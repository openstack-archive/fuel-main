...
$(BUILD_DIR)/mirror-changelog.done: $(BUILD_DIR)/mirror.done
	bash -c "export LOCAL_MIRROR=$(ISOROOT); $(SOURCE_DIR)/report-changelog.sh"
	$(ACTION.TOUCH)

$(BUILD_DIR)/mirror.done:
	@mkdir -p $(ISOROOT)
	packetary --threads-num 10 clone -t deb -r $(UBUNTU_MIRROR_YAML) -d $(ISOROOT)
	packetary --threads-num 10 clone -t rpm -r $(CENTOS_MIRROR_YAML) -R $(SOURCE_DIR)/yaml/packages.yaml -d $(ISOROOT)
	$(ACTION.TOUCH)
