...
if [ -f /root/.build_images ]; then
  ...
  REPO_CONT_ID=$(docker -D run -d -p 80 -v $REPODIR:/var/www/html/centos fuel/centos /usr/sbin/apachectl -d -DFOREGROUND)
  RANDOM_PORT=$(docker port $$REPO_CONT_ID 80)
  ...
  kill `cat /tmp/simple_http_daemon_o${RANDOM_PORT}.pid`
